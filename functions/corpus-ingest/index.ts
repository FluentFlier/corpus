...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (violations.length > 0) {
      ...
      if (violationsError) {
        console.error("Violations insert error:", violationsError);
      }
    }

    // Store graph nodes as memory entries for later retrieval
    if (nodes.length > 0) {
      const memoryRows = nodes.slice(0, 500).map((n: any) => ({
        project_slug: projectSlug,
